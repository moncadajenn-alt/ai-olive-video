 ...
 * Labels blur and spin to exaggerate confusion.
...
const SpinningLabel: React.FC<{ x: number; y: number; text: string; delay: number; color: string }> = ({
  ...
  // spin & blur simulation via rapid rotation
...
export const Scene2: React.FC = () => {
  ...
  // label confusion starts at frame 20
  ...
  return (
    <AbsoluteFill style={{ background: "linear-gradient(160deg, #EEF4FF 0%, #D8E8FF 100%)", overflow: "hidden" }}>
      ...
      {/* ── Scene content zoom wrapper ── */}
      <div style={{ position: "absolute", inset: 0, transform: `scale(${zoom})`, transformOrigin: "center 55%" }}>

        {/* Spinning confused labels */}
        {showLabels && (
          <>
            <SpinningLabel x={80}  y={300} text="Extra Virgin?" color="#4A7C2A" delay={20} />
            <SpinningLabel x={580} y={260} text="Cold Pressed?" color="#8B6914" delay={28} />
            <SpinningLabel x={160} y={600} text="First Press?" color="#2E5C2E" delay={36} />
            <SpinningLabel x={640} y={580} text="Premium?"    color="#9A6E00" delay={44} />
            <SpinningLabel x={320} y={180} text="Organic?"    color="#6C5CE7" delay={32} />
            <SpinningLabel x={500} y={440} text="Pure?"       color="#E17055" delay={24} />
          </>
        )}
        ...
        <div style={{
          ...
          {[
            { color: "#8FBC58", cap: "#4A7C2A", label: "CLASSICO" },
            { color: "#C8A830", cap: "#8B6914", label: "PRIMAVERA" },
            { color: "#5B9B5A", cap: "#2E5C2E", label: "VERDE" },
            { color: "#D4A818", cap: "#9A6E00", label: "DORADO" },
          ].map((b, i) => {
            // bottle 1 (index 1) is being "flipped" / examined
            const isFlipped = i === 1;
            ...
            return (
              <div key={i} style={{ transform: `rotate(${flipAngle}deg)`, transformOrigin: "bottom center" }}>
                <svg width="76" height="130" viewBox="0 0 80 130">
        ...
        <div style={{
          ...
          <BichoCharacter expression={expression} scale={2.5} animate />
        </div>
      </div>
